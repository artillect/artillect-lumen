...
// Combines sun/moon shadowmaps based on time of day
float getBrightness(vec3 Normal) {
    float floatTime = float(worldTime);
    float NdotL;
    ...
    // Sunset
        ...
        float sunsetFrac = (floatTime - 12000.0)/786.0;
        float sunBrightness = max(dot(Normal, normalize(sunPosition)), 0.0);
        float moonBrightness = - 0.2 * max(dot(Normal, normalize(moonPosition)), 0.0);
        NdotL = (1.0 - sunsetFrac) * sunBrightness;
    } 
    // Moonrise
    else if (worldTime >= 12786 && worldTime < 13000) {
        float moonriseFrac = (floatTime - 12786.0)/214.0;
        float moonBrightness = 0.2 * max(dot(Normal, normalize(moonPosition)), 0.0);
        NdotL = moonriseFrac * moonBrightness;
    } 
    // Night
    else if (worldTime >= 13000 && worldTime < 23000) {
        NdotL = 0.2 * max(dot(Normal, normalize(moonPosition)), 0.0);
    }
